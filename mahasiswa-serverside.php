...
if ($_GET['action'] == "table_data") {
    ...
    if (!empty($query)) {
        ...
        while ($value = $query->fetch_array()) {
            ...
            $nestedData['aksi'] = '<div class="text-center" width=19%>
               <a href="detail-mahasiswa.php?id_mahasiswa='.$value['id_mahasiswa'].'" class="btn btn-secondary btn-sm"><i class="far fa-eye"></i> detail</a> 
               <a href="ubah-mahasiswa.php?id_mahasiswa='.$value['id_mahasiswa'].'" class="btn btn-success btn-sm"><i class="fas fa-edit"></i>ubah</a> 
               <a href="hapus-mahasiswa.php?id_mahasiswa='.$value['id_mahasiswa'].'" class="btn btn-danger btn-sm" onclick="return confirm("yakinn ni boy");"><i class="fas fa-trash"></i> hapus</a> 
            </div>';
